...
import rtc.to.rtc2jira.MyProgressMonitor;
...
public class RTCExtractor {
	...
	private void getWorkItems(ITeamRepository repo) throws TeamRepositoryException, IOException {
		IProcessClientService processClientService = (IProcessClientService) repo
		...
		for (IAttribute attribute : allAttributes) {
			if (workItem.hasAttribute(attribute)) {
				...
				String formattedOutput = String.format("Identifier: %s \t Display Name: %s \t Type: %s \t Value: %s",
						attribute.getIdentifier(), attribute.getDisplayName(), attribute.getAttributeType(), value);
	...
	private void saveAttachements(ITeamRepository repo, IWorkItem workItem)
	...
	private void saveAttachment(ITeamRepository teamRepository, IAttachment attachment)
			throws TeamRepositoryException, IOException {
